...
app = typer.Typer(help="Extract trip metrics from archive Parquets into a metrics CSV.")
...
def extract(
    data_dir: Path = typer.Argument(
        ...
        file_okay=False,
        dir_okay=True,
        help="Project directory containing a trips/ sub-folder of archive Parquets.",
    ),
    ...
    run_dt = datetime.now()
    out_dir = data_dir / "analyses" / run_dt.strftime("%Y%m%d-%H%M")
    out_dir.mkdir(parents=True, exist_ok=True)
    out_file = out_dir / "metrics.csv"
    df.to_csv(out_file, index=False)
    typer.secho(f"  Written: {out_file}", fg=typer.colors.GREEN)
    typer.secho("Done. Run 'dcc analyze' to compute similarity scores.", fg=typer.colors.GREEN)
