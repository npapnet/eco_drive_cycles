...
if zero_rows / max(n_states, 1) > 0.5:
    print("  WARNING: > 50% zero rows — consider wider bins (speed_bin_width_kmh in config_syn_cluster.json)")
...
for fname, (frm, to) in per_mt.items():
    ...
    mt_counts = pd.DataFrame({"from": frm, "to": to}).groupby(["from", "to"]).size().unstack(fill_value=0)
