...
class TestComputeSessionMetrics:
    ...
    def test_missing_duration_gives_nan(self):
        df = pd.DataFrame({"Ταχ m/s": [5.0, 6.0]})
    ...
    def test_stop_pct_calculation(self):
        ...
        df = pd.DataFrame({"Ταχ m/s": [0.1, 5.0, 0.1, 5.0]})
        result = compute_session_metrics(df, stop_threshold_kmh=2.0)
...
class TestFindRepresentativeSheet:
    ...
    def test_selects_closest_sheet(self):
        ...
        close_df = pd.DataFrame({
            "Διάρκεια (sec)": [600.0, 600.0, 600.0],
            "Ταχ m/s": [10.0, 12.0, 11.0],
            "Επιταχυνση": [0.5, 0.6, 0.55],
            "Επιβραδυνση": [-0.4, -0.5, -0.45],
        })
        far_df = pd.DataFrame({
            "Διάρκεια (sec)": [1.0, 1.0, 1.0],
            "Ταχ m/s": [0.1, 0.1, 0.1],
            "Επιταχυνση": [0.01, 0.01, 0.01],
            "Επιβραδυνση": [-0.01, -0.01, -0.01],
        })
    ...
    def test_score_is_in_valid_range(self):
        df1 = pd.DataFrame({"Ταχ m/s": [5.0, 10.0], "Επιταχυνση": [0.3, 0.3], "Επιβραδυνση": [-0.2, -0.2]})
        df2 = pd.DataFrame({"Ταχ m/s": [8.0, 12.0], "Επιταχυνση": [0.4, 0.4], "Επιβραδυνση": [-0.3, -0.3]})
